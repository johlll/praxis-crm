-- A8 — integra `clients` ao merge/undo de contatos da A3/A4. `clients`
-- não existia quando merge_contacts()/unmerge_contact() foram escritas
-- (só nasceu na A5, depois até da integração de `leads` na A4) — sem
-- isso, mesclar um contato que já é cliente deixaria `clients.contact_id`
-- "preso" ao contato perdedor (que passa a responder como inativo em
-- `contacts`), quebrando "Ver cliente" a partir do contato vencedor e a
-- consulta de histórico do cliente por contato.
--
-- Mesma assinatura, mesmo GRANT de antes (não muda) — CREATE OR REPLACE
-- só troca o corpo. Reparenta TODO client_handoffs — wait, não:
-- client_handoffs referencia client_id, não contact_id, então já segue o
-- cliente automaticamente sem reparentamento próprio; só `clients.
-- contact_id` precisa mover.
--
...
-- contact_id do vencedor colidiria com o índice único parcial
-- clients_one_active_per_contact_idx (A5) — dois clientes ativos para o
-- MESMO contact_id. Nunca mesclado nem apagado silenciosamente (mesmo
-- princípio de ADR-007: nenhuma união automática de entidade sem decisão
-- humana): a mesclagem de CONTATO é recusada com erro tratado
-- ('client_merge_conflict_active_client') até que um administrador
-- decida manualmente qual dos dois clientes ativos deve ceder (mudando
-- o status de um deles primeiro, fora desta função). Clientes NÃO
-- ativos (encerrado/suspenso) do perdedor sempre podem mover — o índice
-- só restringe status = 'ativo'.
