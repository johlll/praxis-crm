...
export function ActivitiesPagination({ page, total, pageSize, currentParams }: ActivitiesPaginationProps) {
  ...
  function href(targetPage: number) {
    ...
    return `/atividades?${params.toString()}` as never;
  }

  return (
    <div className="flex items-center justify-between text-meta text-text-tertiary">
      <span>
        Página {page} de {totalPages} — {total} {total === 1 ? "atividade" : "atividades"}
      </span>
