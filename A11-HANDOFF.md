...
**Este documento passou por DUAS rodadas de auditoria pós-dry-run.** A
...
sem formato de UUID) — corrigidos e registrados em §2.2. Todos os 20,
...
Nenhuma migration **já aplicada** foi editada em nenhuma das duas
...
- **`answers_config`/snapshot corrompidos falham fechados em três
  camadas** — borda, `ingest_form_event` e worker (item 4 da 2ª rodada).
...
|---|---|---|
| `npm test` (Vitest, todos os arquivos) | **402/402 passando** | **execução local** nesta sessão |
| — dos quais, arquivos `tests/unit/a11-*.test.ts` | 7 arquivos (novo: `a11-worker.test.ts`) | execução local |
| `supabase/tests/database/18_a11_ingestao_atribuicao.test.sql` | **127 asserções** (contagem estática — 113 depois da 1ª rodada, 70 antes dela) | **CI** (pendente nesta sessão) |
| `tests/e2e/forms-attribution.spec.ts` | **15 testes** (contagem estática — 14 depois da 1ª rodada, 13 antes dela) | **CI** (pendente nesta sessão) |
...
Os e2e e pgTAP da primeira rodada seguem cobertos — nenhuma asserção foi
enfraquecida ou removida, só reescrita quando o próprio mecanismo que ela
verificava mudou (ex.: o hash da referência de continuidade).
...
atualizado à mão para a coluna nova
(`continuity_references.updated_at`); é o CI, com Docker, que confirma se
o arquivo bate com o que `supabase gen types` produziria.
...
20260921100600_a11_attribution_functions.sql correção de vínculo (por vínculo efetivo) + emissão/revogação de continuidade (hash de texto, alcance por registro)
